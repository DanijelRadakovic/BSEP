...
public interface CertificateRepository extends JpaRepository<Certificate, Long> {
    ...
    @Query(value = "SELECT * FROM certificate WHERE type = 0 or type = 1", nativeQuery = true)
    List<Certificate> findAllCA();

    @Query(value = "SELECT * FROM certificate WHERE active = true and (type = 0 or type = 1)", nativeQuery = true)
    List<Certificate> findAllActiveCA();

    @Query(value = "SELECT * FROM certificate WHERE type = 2", nativeQuery = true)
    List<Certificate> findAllClients();

    @Query(value = "SELECT * FROM certificate WHERE active = true and type = 2", nativeQuery = true)
